...
for i in range(img.size[0]):    # for every pixel:
    for j in range(img.size[1]):
    	...
    	formulaR = int(((math.sin(math.pi*float(i)/float(periodR)+deltaR)+1)/2)*((math.cos(math.pi*float(j)/float(periodR)+deltaR)+1)/2)*255)
    	formulaG = int(((math.cos(math.pi*float(i)/float(periodG)+deltaG)+1)/2)*((math.cos(math.pi*float(j)/float(periodG)+deltaG)+1)/2)*255)
    	formulaB = int(((math.cos(math.pi*float(i)/float(periodB)+deltaB)+1)/2)*((math.cos(math.pi*float(j)/float(periodB)+deltaB)+1)/2)*255)
