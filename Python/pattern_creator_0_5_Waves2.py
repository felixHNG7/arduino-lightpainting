...
for i in range(img.size[0]):    # for every pixel:
    for j in range(img.size[1]):
    	...
    	formulaR = int(((math.cos(math.pi*float(j)/float(periodRVert))+math.cos(math.pi*float(i)/periodRHori))/2)*255)
    	formulaG = int(((math.cos(math.pi*float(j)/float(periodGVert))+math.cos(math.pi*float(i)/periodGHori))/2)*255)
    	formulaB = int(((math.cos(math.pi*float(j)/float(periodBVert))+math.cos(math.pi*float(i)/periodBHori))/2)*255)
