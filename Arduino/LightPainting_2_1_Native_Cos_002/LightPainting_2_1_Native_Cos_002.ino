...
Serial.println(roof_intensity);
  ...
  Serial.println(x);
   ...
   { 
     rArray[i]= ((cos(M_PI*i*i/144)+1)/2*(cos(M_PI*x*x/128)+1)/2)*roof_intensity;
      if (rArray[i]>roof_intensity) rArray[i]=0;
 
      gArray[i]= (cos(M_PI*i/72)+cos(M_PI*x/64)+1)/2*roof_intensity;
      if (gArray[i]>roof_intensity) gArray[i]=0;
      
      

      bArray[i]= ((cos(M_PI*(x-i)/32)+1)/2+(cos(M_PI*(x+i)/128)+1)/2)/2*roof_intensity;
      if (bArray[i]>roof_intensity) bArray[i]=0;
